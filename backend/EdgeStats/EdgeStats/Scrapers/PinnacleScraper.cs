...
namespace EdgeStats.Scrapers
{
	public class PinnacleScraper : ISportsbookScraper
	{
		...
		public async Task Scrape(List<string> leagues)
		{
			...
			try
			{
				foreach (var leagueCode in leagues)
				{
					...
					{
						var (scrapedProps, scrapedLines) = GetProps(driver, wait, game);
		...
		private (List<ScrapedPropDto> props, List<ScrapedLineDto> lines) GetProps(
			ChromeDriver driver,
			WebDriverWait wait,
			Game game
			)
			...
			try
			{
				...
				foreach (var propCategory in propCategoryDivs)
				{
					...
					foreach (var propDiv in propDivs)
					{
						...
						var lines = GetLines(driver, wait, lineDivs, propDto, game, team1, team2);
		...
		private List<ScrapedLineDto> GetLines(
			ChromeDriver driver,
			WebDriverWait wait,
