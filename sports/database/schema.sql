...
CREATE TABLE GAME_URL (
    ...
    game_id INT NOT NULL REFERENCES GAME(game_id) ON DELETE CASCADE,
    sportsbook_id INT NOT NULL REFERENCES SPORTSBOOK(sportsbook_id) ON DELETE CASCADE,
    game_url VARCHAR(2048) NOT NULL UNIQUE
);

-- Indexes for performance
CREATE INDEX idx_prop_game_id ON PROP (game_id);
CREATE INDEX idx_gameurl_game_id ON GAME_URL (game_id);
CREATE INDEX idx_gameurl_sportsbook_id ON GAME_URL (sportsbook_id);
CREATE INDEX idx_team_league_id ON TEAM (league_id);
CREATE INDEX idx_games_league_id ON GAME (league_id);
CREATE INDEX idx_sportsbook_league_url_league_id ON SPORTSBOOK_LEAGUE_URL (league_id);

-- Additional Indexes for UUID columns
CREATE INDEX idx_games_game_uuid ON GAME (game_uuid);
